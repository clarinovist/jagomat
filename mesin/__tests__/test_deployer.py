...
class RunnerPalsu:
    ...
    def __call__(self, argv, **kwargs):
        ...
        if a[0] == "pull":
            ...
            hasil = "OSN_SCHEMA_ADMIN6_AI2_OK"
        else:
...
def test_script_health_sql_readonly_dengan_db_sintetis(tmp_path, versi, ledger, expected):
    ...
    (tmp_path / 'admin_store.py').write_text("VERSI_SKEMA = 6\nraise RuntimeError('jangan import')\n")
...
def test_probe_image_synthetic_subprocess_python_saja(tmp_path):
    ...
    assert hasil.stdout.strip() == "OSN_IMAGE_ADMIN6_AI2_OK"
...
def test_probe_image_menolak_kontrak_admin_rusak(tmp_path, fault):
    ...
    assert 'OSN_IMAGE_ADMIN6_AI2_OK' not in hasil.stdout
