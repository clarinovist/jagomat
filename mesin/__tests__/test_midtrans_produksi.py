"""Transport produksi tanpa socket nyata; paritas kebijakan dengan adapter sandbox."""
...
def jaringan(monkeypatch):
    ...
    monkeypatch.setattr(p.http.client, "HTTPSConnection", baru)
    return koneksi, dibuat


def test_query_produksi_host_tls_timeout_dan_close(jaringan):
    koneksi, dibuat = jaringan
    hasil = m.periksa_status(CFGP, INV, akun_id=AKUN, transport=p.TransportProduksi(CFGP), sakelar=ON)
    assert hasil.status == "lunas" and hasil.bukti.invoice_id == INVOICE
    assert len(dibuat) == 1 and dibuat[0][:2] == ("api.midtrans.com", 10)
    konteks = dibuat[0][2]
    ...
    assert koneksi.panggilan[0] == ("GET", "/v2/" + INVOICE + "/status", None,
                                    m.request_status(CFGP, INVOICE).headers)
    assert all(0 < t <= 10 for t in koneksi.timeout)
    assert koneksi.ditutup and koneksi.respons.ditutup


def test_sandbox_ditolak_sebelum_socket(jaringan):
    with pytest.raises(m.KontrakTidakSah):
        p.TransportProduksi(CFG)
    with pytest.raises(m.KontrakTidakSah):
        p.TransportProduksi("bukan konfigurasi")
    assert jaringan[1] == []


def test_create_minimal_produksi_tidak_grant(jaringan):
    koneksi, _ = jaringan
    data = status()
    data.update(status_code="201", transaction_status="pending", actions=[{
        "name": "generate-qr-code", "method": "GET",
        "url": "https://api.midtrans.com/v2/qris/trx_sintetis/qr-code"}])
    koneksi.respons.data = json.dumps(data).encode()
    hasil = m.buat_pembayaran(CFGP, INV, akun_id=AKUN, transport=p.TransportProduksi(CFGP), sakelar=ON)
    ...
    assert headers == m.request_create(CFGP, INVOICE, 15000, INV["idempotency_key"]).headers


def test_qr_sandbox_tidak_dianggap_produksi(jaringan):
    koneksi, _ = jaringan
    data = status()
    data.update(status_code="201", transaction_status="pending", actions=[{
        "name": "generate-qr-code", "method": "GET",
    ...
    koneksi.respons.data = json.dumps(data).encode()
    hasil = m.buat_pembayaran(CFGP, INV, akun_id=AKUN, transport=p.TransportProduksi(CFGP), sakelar=ON)
    assert hasil.qr is None and hasil.bukti is None


@pytest.mark.parametrize("ubah", [
    {"url": "https://api.sandbox.midtrans.com/v2/" + INVOICE + "/status"},
    {"url": "https://api.midtrans.com.evil.test/v2/" + INVOICE + "/status"},
    {"url": "http://api.midtrans.com/v2/" + INVOICE + "/status"},
    {"url": "https://api.midtrans.com:443/v2/" + INVOICE + "/status"},
    {"url": "https://api.midtrans.com/v2/" + INVOICE + "/status?secret=x"},
...
def test_request_asing_ditolak_sebelum_socket(jaringan, ubah):
    req = replace(m.request_status(CFGP, INVOICE), **ubah)
    with pytest.raises(m.KontrakTidakSah):
        m.kirim(req, transport=p.TransportProduksi(CFGP))
    assert jaringan[1] == []


@pytest.mark.parametrize("jenis", ["kontak", "duplikat", "header", "key"])
def test_create_tambahan_ditolak_sebelum_socket(jaringan, jenis):
    req = m.request_create(CFGP, INVOICE, 15000, INV["idempotency_key"])
...
def test_opsi_tidak_aman_ditolak(jaringan, timeout, redirect):
    with pytest.raises(m.KontrakTidakSah):
        with p.TransportProduksi(CFGP)(m.request_status(CFGP, INVOICE), timeout=timeout, allow_redirects=redirect):
            pass
    assert jaringan[1] == []


@pytest.mark.parametrize("kode", [201, 301, 302, 307, 308, 401, 500])
def test_error_redirect_tanpa_read_retry_atau_grant(jaringan, kode):
    ...
    assert m.periksa_status(CFGP, INV, akun_id=AKUN, transport=p.TransportProduksi(CFGP), sakelar=ON).bukti is None
...
def test_header_tidak_sah_tanpa_baca(jaringan, header, nilai):
    ...
    with pytest.raises(m.KontrakTidakSah):
        m.kirim(m.request_status(CFGP, INVOICE), transport=p.TransportProduksi(CFGP))
...
def test_baca_bounded_meski_tanpa_content_length(jaringan):
    ...
    with pytest.raises(m.KontrakTidakSah):
        with p.TransportProduksi(CFGP)(m.request_status(CFGP, INVOICE)):
...
def test_deadline_memutus_stream_lambat(jaringan, monkeypatch):
    ...
    monkeypatch.setattr(p.time, "monotonic", lambda: next(waktu))
    with pytest.raises(m.KontrakTidakSah):
        with p.TransportProduksi(CFGP)(m.request_status(CFGP, INVOICE)):
...
def test_gambar_hanya_png_produksi_bounded(jaringan):
    koneksi, dibuat = jaringan
    ...
    url = "https://api.midtrans.com/v2/qris/trx_sintetis/qr-code"
    assert p.TransportProduksi(CFGP).gambar(url) == png
    assert dibuat[0][0] == "api.midtrans.com"
    for asing in ("https://api.sandbox.midtrans.com/v2/qris/trx_sintetis/qr-code",
                  "https://evil.test/v2/qris/trx_sintetis/qr-code",
                  "https://api.midtrans.com/v3/qris/trx_sintetis/qr-code"):
        with pytest.raises(m.KontrakTidakSah):
            p.TransportProduksi(CFGP).gambar(asing)
    assert len(dibuat) == 1


def test_sakelar_off_dan_owner_sebelum_jaringan(jaringan):
    tr = p.TransportProduksi(CFGP)
    for fungsi in (m.buat_pembayaran, m.periksa_status):
        with pytest.raises(d.FiturNonaktif, match="fitur langganan nonaktif"):
            fungsi(CFGP, INV, akun_id=AKUN, transport=tr)
        with pytest.raises(LookupError):
            fungsi(CFGP, INV, akun_id="akun_" + "f" * 32, transport=tr, sakelar=ON)
    assert jaringan[1] == []
