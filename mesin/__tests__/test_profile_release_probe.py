...
def siap(tmp_path):
    ...
    source.mkdir()
    for nama, versi in [('admin_store', 5), ('assistant_schema', 4), ('ai_store', 2)]:
...
def test_readiness_admin5_memeriksa_metadata_dan_tidak_menulis(siap, rusak, tmp_path):
    ...
    if rusak in ('admin4', 'admin6'):
        with sqlite3.connect(data / 'admin-control.db') as kon:
            kon.execute('PRAGMA user_version=' + ('4' if rusak == 'admin4' else '6'))
    elif rusak == 'source4':
    ...
    if rusak is None:
        ...
        assert 'OSN_SCHEMA_ADMIN5_AI2_OK' not in hasil.stdout
...
def test_ringkasan_candidate_wajib_mengaku_enam_probe_profil_dan_admin5():
    ...
    assert hasil['profil_checks'] == 6 and hasil['admin_schema'] == 5
