...
def _pager(sid, filter_data, total):
    jumlah = max(1,(total+H.PER_HALAMAN-1)//H.PER_HALAMAN)
    kini = filter_data.halaman
    nomor = sorted({1,jumlah} | set(range(max(1,kini-1),min(jumlah,kini+2)+1)))
    bagian=[]
    if kini>1:
        bagian.append('<a href="%s">← Sebelumnya</a>' % _e(filter_data.tautan(sid,kini-1)))
    lalu=0
    for n in nomor:
        if lalu and n>lalu+1:
            bagian.append('<span>…</span>')
        bagian.append('<span aria-current="page">%d</span>' % n if n==kini else '<a href="%s">%d</a>' % (_e(filter_data.tautan(sid,n)),n))
        lalu=n
    if kini<jumlah:
        bagian.append('<a href="%s">Berikutnya →</a>' % _e(filter_data.tautan(sid,kini+1)))
...
def riwayat(siswa_id, baris, total, filter_data, *, judul_topik, tanggal, badge_tinjauan, aksi_bagikan):
    ...
    filter_html = ('<form method="get" action="/anak/%d" class="profil-filter-st">'
                   ...
                   '<button type="submit" class="st-tombol-sekunder">Terapkan filter</button></form>') % (
                       siswa_id,_e(f.mulai),_e(f.sampai),_opsi([('','Semua topik')]+[(k,ambil(k).nama) for k in daftar_topik()],f.topik),
    ...
    for r in baris:
        ...
        meta='%s · %s · %s · Sesi #%d' % (jenis,label_kelas(r['level']),mode,r['id'])
        isi.append('<tr data-sesi-id="%d"><td class="riwayat-tanggal-st">%s</td>'
                   '<td class="riwayat-latihan-st"><strong>%s</strong><small>%s</small>%s</td>'
                   ...
                   '<td class="riwayat-tinjauan-st">%s</td><td class="riwayat-aksi-st"><a href="/sesi/%d" aria-label="Buka sesi %d">Buka →</a>%s</td></tr>' % (
                       r['id'],tanggal(r['tanggal']),_e(judul),_e(meta),('<small>'+rincian+'</small>') if rincian else '',
                       r['n'],_e(proses),tinjauan,r['id'],r['id'],aksi_bagikan(r)))
    if not isi:
        isi.append('<tr><td colspan="6">Tidak ada sesi yang cocok. Ubah filter atau buat latihan baru.</td></tr>')
    ...
    return ('<section aria-labelledby="judul-riwayat"><div class="kepala-riwayat-st"><h2 class="st" id="judul-riwayat">Riwayat latihan</h2>'
            ...
            '<div class="profil-arsip-st" id="filter-riwayat">%s<div class="profil-paging-st"><span>%s<br>Terbaru dahulu · 20 sesi per halaman</span>%s</div>'
            ...
            '</tr></thead><tbody>%s</tbody></table></div><div class="profil-paging-st"><span>%s</span><a href="#filter-riwayat">Kembali ke filter &amp; halaman ↑</a></div></div></section>') % (
                siswa_id,filter_html,hitung,_pager(siswa_id,f,total),''.join(isi),hitung)
...
.profil-workspace-st .profil-champs-st {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:{T.SP_4}; min-width:0; }}
...
.profil-workspace-st .profil-filter-st input,.profil-workspace-st .profil-filter-st select {{ width:100%; min-width:0; min-height:{T.TARGET_SENTUH}; padding:{T.SP_2}; font:inherit; border:1px solid {T.BORDER_VARIAN}; background:{T.LATAR_KARTU}; border-radius:{T.RADIUS_KECIL}; }}
...
@media(min-width:64rem) {{
 ...
 .profil-workspace-st .profil-formulaire-st:has(.pendamping-inline) .strip-sesi.profil-manuel-st > .profil-champs-st {{ grid-column:1; }}
...
@media(max-width:48rem) {{
 ...
 .profil-workspace-st .profil-filter-st {{ grid-template-columns:repeat(2,minmax(0,1fr)); padding:{T.SP_4}; }}
 ...
 .profil-workspace-st .tabel-riwayat-st tr {{ display:grid; grid-template-columns:minmax(0,1fr) auto; gap:{T.SP_2}; padding:{T.SP_4}; border-bottom:1px solid {T.BORDER_HALUS}; }}
 .profil-workspace-st .tabel-riwayat-st td {{ padding:0; border:0; }}
 ...
 .profil-workspace-st .riwayat-proses-st::before {{ content:'Pengerjaan: '; color:{T.TEKS_VARIAN}; font-size:.7rem; }}
 .profil-workspace-st .riwayat-tinjauan-st {{ grid-column:1; grid-row:4; }}
 .profil-workspace-st .riwayat-tinjauan-st::before {{ content:'Tinjauan: '; color:{T.TEKS_VARIAN}; font-size:.7rem; }}
 .profil-workspace-st .riwayat-aksi-st {{ grid-column:2; grid-row:3/5; }}
}}
"""
